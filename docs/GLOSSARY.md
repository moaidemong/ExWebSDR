...
- **FFT row**: A single frequency-domain representation computed from a window of IQ samples using Fast Fourier Transform. Contains power spectrum data for 8192 frequency bins, converted to dB scale.
...
- **Backpressure**: The condition when data is being produced faster than it can be consumed. In ExWebSDR, handled by dropping stale frames to maintain real-time streaming without accumulating delays.
